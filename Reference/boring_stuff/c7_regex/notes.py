...
regex_object.search('search string'), when search is called on a regex object, it returns a Match Object if there are any.
...
Search() only finds the first matched pattern, if there are groups in the regex object it returns a tuple with the groups.

You can group different parts of the search pattern as below;
groups() refers to the tuple of all the groups found in the matched pattern.
group() will return a single string of the entire matched pattern, same for group(0)
Groups(1) will be the first group, in the example below it would be the 3 character extension;
"""
phone_num_regex = re.compile(r'(\d\d\d)-(\d\d\d-\d\d\d\d)')
mo = phone_num_regex.search('My number is 999-754-9997')
# print(mo.groups())  # this is a tuple of multiple values. ('999', '754-9997')
# print(mo.groups(0)) #('999', '754-9997'), same as groups()
# print(mo.group(1)) # prints 999, which is the first group.
# print(mo.group(2)) # prints 754-9997
# print(mo.group()) # .group prints 999-754-9997.
# print(mo.group(0)) # 999-754-9997, sme as above.
...
mo = batRegex.search('The Batmobile lost a wheel..., along with the Batcopter')
...
# print(mo2.groups()) # (None, '555-4213')
...
mo2 = phoneNumRegex.search('not here!!')
...
# update: once I include ' ' and '!' in the regex character class, it selects the correct conosants


# my own implemenation
...
The dot star uses greedy model it will always try to match as much text as possible.
...
mo1 = newline_regex.search(
    'Serve the public trust.\nProtect the innocent. \nUphold the law.')
# print(mo1.group()) #prints in multiple lines, the full text.
...
# result = agent_names_regex.sub(r'\1****', 'Agent 123 and Agent Alice told Agent Carol that Agent Eve knew Agent Bob was a double agent.')
# print(result) # result is a string that prints '1**** and A**** told C**** that E**** knew B**** was a double agent.'
...
"""
#######################################   managing complex regex with .VERBOSE   #######################################
...
Done - Practice project: Strong Password Detection - 05 jan
Done - Practice project: regex version of strip() - 04 jan
Done - Practice project: sensitive info (social security, credit card number) censoring engine; roughly done
Done - Self project: URL detector - 06 jan
Ongoing, fun. - self project: ether address and bitcoin address detector and extractor; roughly done + seed phrase detector
